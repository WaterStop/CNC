/********************************************************************************
** Form generated from reading UI file 'widget3_15.ui'
**
** Created by: Qt User Interface Compiler version 5.14.2
**
** WARNING! All changes made in this file will be lost when recompiling UI file!
********************************************************************************/

#ifndef UI_WIDGET3_15_H
#define UI_WIDGET3_15_H

#include <QtCore/QVariant>
#include <QtWidgets/QApplication>
#include <QtWidgets/QFrame>
#include <QtWidgets/QPushButton>
#include <QtWidgets/QWidget>
#include "mylineedit.h"

QT_BEGIN_NAMESPACE

class Ui_widget3_15
{
public:
    QFrame *frame;
    MyLineEdit *edit_2;
    QPushButton *bt_2;
    MyLineEdit *edit_3;
    QPushButton *bt_1;
    MyLineEdit *edit_1;
    QPushButton *bt_close;
    QPushButton *bt_lastpage;

    void setupUi(QWidget *widget3_15)
    {
        if (widget3_15->objectName().isEmpty())
            widget3_15->setObjectName(QString::fromUtf8("widget3_15"));
        widget3_15->resize(293, 430);
        frame = new QFrame(widget3_15);
        frame->setObjectName(QString::fromUtf8("frame"));
        frame->setGeometry(QRect(0, 0, 293, 430));
        QSizePolicy sizePolicy(QSizePolicy::Ignored, QSizePolicy::Ignored);
        sizePolicy.setHorizontalStretch(0);
        sizePolicy.setVerticalStretch(0);
        sizePolicy.setHeightForWidth(frame->sizePolicy().hasHeightForWidth());
        frame->setSizePolicy(sizePolicy);
        frame->setFrameShape(QFrame::NoFrame);
        frame->setFrameShadow(QFrame::Plain);
        frame->setLineWidth(0);
        edit_2 = new MyLineEdit(frame);
        edit_2->setObjectName(QString::fromUtf8("edit_2"));
        edit_2->setGeometry(QRect(70, 200, 113, 31));
        sizePolicy.setHeightForWidth(edit_2->sizePolicy().hasHeightForWidth());
        edit_2->setSizePolicy(sizePolicy);
        QFont font;
        font.setFamily(QString::fromUtf8("\345\276\256\350\275\257\351\233\205\351\273\221"));
        font.setPointSize(12);
        edit_2->setFont(font);
        edit_2->setStyleSheet(QString::fromUtf8("color: rgb(255, 255, 255);\n"
"background-color: rgb(64, 86, 129);"));
        edit_2->setAlignment(Qt::AlignRight|Qt::AlignTrailing|Qt::AlignVCenter);
        bt_2 = new QPushButton(frame);
        bt_2->setObjectName(QString::fromUtf8("bt_2"));
        bt_2->setGeometry(QRect(100, 160, 61, 31));
        sizePolicy.setHeightForWidth(bt_2->sizePolicy().hasHeightForWidth());
        bt_2->setSizePolicy(sizePolicy);
        edit_3 = new MyLineEdit(frame);
        edit_3->setObjectName(QString::fromUtf8("edit_3"));
        edit_3->setGeometry(QRect(70, 250, 113, 31));
        sizePolicy.setHeightForWidth(edit_3->sizePolicy().hasHeightForWidth());
        edit_3->setSizePolicy(sizePolicy);
        edit_3->setFont(font);
        edit_3->setStyleSheet(QString::fromUtf8("color: rgb(255, 255, 255);\n"
"background-color: rgb(64, 86, 129);"));
        edit_3->setAlignment(Qt::AlignRight|Qt::AlignTrailing|Qt::AlignVCenter);
        bt_1 = new QPushButton(frame);
        bt_1->setObjectName(QString::fromUtf8("bt_1"));
        bt_1->setGeometry(QRect(100, 110, 61, 31));
        sizePolicy.setHeightForWidth(bt_1->sizePolicy().hasHeightForWidth());
        bt_1->setSizePolicy(sizePolicy);
        edit_1 = new MyLineEdit(frame);
        edit_1->setObjectName(QString::fromUtf8("edit_1"));
        edit_1->setGeometry(QRect(70, 60, 113, 31));
        sizePolicy.setHeightForWidth(edit_1->sizePolicy().hasHeightForWidth());
        edit_1->setSizePolicy(sizePolicy);
        edit_1->setFont(font);
        edit_1->setStyleSheet(QString::fromUtf8("color: rgb(255, 255, 255);\n"
"background-color: rgb(64, 86, 129);"));
        edit_1->setAlignment(Qt::AlignRight|Qt::AlignTrailing|Qt::AlignVCenter);
        bt_close = new QPushButton(frame);
        bt_close->setObjectName(QString::fromUtf8("bt_close"));
        bt_close->setGeometry(QRect(260, 10, 20, 20));
        sizePolicy.setHeightForWidth(bt_close->sizePolicy().hasHeightForWidth());
        bt_close->setSizePolicy(sizePolicy);
        bt_lastpage = new QPushButton(frame);
        bt_lastpage->setObjectName(QString::fromUtf8("bt_lastpage"));
        bt_lastpage->setGeometry(QRect(20, 380, 20, 20));
        sizePolicy.setHeightForWidth(bt_lastpage->sizePolicy().hasHeightForWidth());
        bt_lastpage->setSizePolicy(sizePolicy);

        retranslateUi(widget3_15);

        QMetaObject::connectSlotsByName(widget3_15);
    } // setupUi

    void retranslateUi(QWidget *widget3_15)
    {
        widget3_15->setWindowTitle(QCoreApplication::translate("widget3_15", "Form", nullptr));
        bt_2->setText(QString());
        bt_1->setText(QString());
        bt_close->setText(QString());
        bt_lastpage->setText(QString());
    } // retranslateUi

};

namespace Ui {
    class widget3_15: public Ui_widget3_15 {};
} // namespace Ui

QT_END_NAMESPACE

#endif // UI_WIDGET3_15_H
