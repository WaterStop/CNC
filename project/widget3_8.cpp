...
void widget3_8::loadWorkInfo(int index)
{
    ...
    if(cur_work_mod==duanmian2)
    {
        ...
        ui->edit_6->setText(str);
        dir_1 = tem_val->endFace2.zDir;
        dir_2 = tem_val->endFace2.xDir;
        change_axisdir_icon();
    }
}
...
bool widget3_8::wedget3_8ToProcessList(MachineProcess* dealInterfaceData, s_endFaceMode2* endFace2, Ui::widget3_8* ui)
{
    ...
    else
    {
        if(modify_flg==1)
        {
            dealInterfaceData->changeNode(cur_Node, *endFace2);
        }
        else  if (false == dealInterfaceData->addNode(*endFace2))
        {
            return false;
        }
    }
